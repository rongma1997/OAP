 ...
 protected:
  ...
  void doSplit() {
    ...
    do {
      ...
      if (record_batch) {
        auto array = rand.Numeric<arrow::Int32Type>(record_batch->num_rows(), 0, 10);
    ...
    std::cout << "BenchmarkExtractBigScale processed " << num_batches << " batches, took "
              << TIME_TO_STRING(elapse_read) << " doing Batch Read, took "
              << TIME_TO_STRING(elapse_split) << " doing Batch split, took "
              << TIME_TO_STRING(elapse_write) << " doing Batch write" << std::endl;
